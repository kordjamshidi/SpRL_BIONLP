        ...
        for (int Hindex1 = 0; Hindex1 < habitatList.size(); Hindex1++)
            for (int Hindex2 = 0; Hindex2 < habitatList.size(); Hindex2++) {
                if (Hindex1 != Hindex2) {
                    ...
                    else if (discourse_annotations.entities.elementAt(h1).sentence_id < discourse_annotations.entities.elementAt(h2).sentence_id)
                        break; //h2 is in a senentece after h1, so stop searching
            ...
            for (int j = 0; j < allsentences.elementAt(i).sen_chuncks.size(); j++) {
                ...
                if (!Util.d2Int_d2_Int(entityList.elementAt(i), allsentences.elementAt(i).sen_chuncks.elementAt(j).wordIndexes)) {
                    ...
                    if (ws.pos.contains("IN") | ws.pos.contains("VB")) {
                        ...
                        //  }
                    } else if (!inVerbPhrase(temp, allsentences.elementAt(i).parseTree)) {
                        String w = allsentences.elementAt(i).sentence_feat.elementAt(temp[0]).words.word;
        ...
        for (int i = 0; i < discourse_annotations.entities.size(); i++) {
            ...
            if (e.role.isEmpty())
                if (FEx_BIONLP_BB_PerSentence_train_test_MoreGlobalBMC.obRep.exists_Habitat_inOBO(S1))
                    lm_candidates.add(e);
                else if (FEx_BIONLP_BB_PerSentence_train_test_MoreGlobalBMC.NCBIRep.exists_Bactriun_NCBI(S1))
                    tr_candidates.add(e);
                else
                ...
                for (int i = sen_num; i < allsentences.size(); i++) {
                    if (par.contains(allsentences.elementAt(i).content)) {
                        ...
                        if (par.endsWith(allsentences.elementAt(i).content)) {
                            sen_num = i + 1;
                            break;
                        } // the whole paragraph is parsed and matched with sentences
                    }
                }
            }
...
//}
    ...
    public void writeDiscourseXML(PrintWriter spxml, int doc_num, int a1Flag, int a2Flag) {
        ...
        for (int i = 0; i < allsentences.size(); i++) {
            ...
            spxml.print("    <CONTENT>");
            spxml.print(allsentences.elementAt(i).content);
...
// }// end write Entity/Relationship features prosedure
	...
	/* hashTable is to collect the features in a structured way, hashTable-0 is the entity features and hashTable-1 are the relationship feature*/
        ...
        for (int r = 0; r < discourse_annotations.relations.size(); r++) {
            ...
            int e1 = find_entity(discourse_annotations.relations.elementAt(r).id1);
            int e2 = find_entity(discourse_annotations.relations.elementAt(r).id2);
            phrase_annotations B1 = discourse_annotations.entities.elementAt(e1);
            phrase_annotations H1 = discourse_annotations.entities.elementAt(e2);
            String[] S1 = {}, S2 = {};
            for (int w = 0; w < B1.wordIndexes.length; w++) {
                S1 = Arrays.copyOf(S1, S1.length + 1);
                S1[S1.length - 1] = PreProcessing.BacteriaToken(allsentences.elementAt(B1.sentence_id).sentence_feat.elementAt(B1.wordIndexes[w]).words.word);
            }
            for (int w = 0; w < H1.wordIndexes.length; w++) {
                S2 = Arrays.copyOf(S2, S2.length + 1);
                S2[S2.length - 1] = PreProcessing.BacteriaToken(allsentences.elementAt(H1.sentence_id).sentence_feat.elementAt(H1.wordIndexes[w]).words.word);
            }
            ...
            if (Insertflag == 1) {
                ...
                } else if (label.toLowerCase().equals("localization")) {
                ...
                } else if (label.toLowerCase().equals("localizationcoref")) {
...
*/
		/*	String id1=discourse_annotations.relations.elementAt(r).id1;
            ...
            for (int r2 = r + 1; r2 < discourse_annotations.relations.size(); r2++) {

                int e3 = find_entity(discourse_annotations.relations.elementAt(r2).id1);
                int e4 = find_entity(discourse_annotations.relations.elementAt(r2).id2);

                phrase_annotations B2 = discourse_annotations.entities.elementAt(e3);
                phrase_annotations H2 = discourse_annotations.entities.elementAt(e4);
                String[] S3 = {}, S4 = {};
                for (int w = 0; w < B2.wordIndexes.length; w++) {
                    S3 = Arrays.copyOf(S3, S3.length + 1);
                    S3[S3.length - 1] = PreProcessing.BacteriaToken(allsentences.elementAt(B2.sentence_id).sentence_feat.elementAt(B2.wordIndexes[w]).words.word);
                }
                for (int w = 0; w < H2.wordIndexes.length; w++) {
                    S4 = Arrays.copyOf(S4, S4.length + 1);
                    S4[S4.length - 1] = PreProcessing.BacteriaToken(allsentences.elementAt(H2.sentence_id).sentence_feat.elementAt(H2.wordIndexes[w]).words.word);
                }
                ...
                rr_count++;
                if (!(discourse_annotations.relations.elementAt(r2).relationship.startsWith("P") || discourse_annotations.relations.elementAt(r).relationship.startsWith("P"))) {
                    ...
                    if (e1 == e3)
                        pw.R_RFeatures.println((rr_count) + "\t" + (hashTable.elementAt(2).indexOf("same_T") + 1) + "\t" + "1");
                    double sim = StringSimilarity.Jaccard(S1, S3);
                    pw.R_RFeatures.println((rr_count) + "\t" + (hashTable.elementAt(2).indexOf("TT_sim") + 1) + "\t" + sim);
                    sim = StringSimilarity.Jaccard(S2, S4);
                    pw.R_RFeatures.println((rr_count) + "\t" + (hashTable.elementAt(2).indexOf("LL_sim") + 1) + "\t" + sim);
